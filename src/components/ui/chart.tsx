...
// /* ---------------- Tooltip ----------------
//    We create explicit prop types that use the Recharts TooltipProps' payload & label types,
//    while allowing normal HTML div attributes via React.HTMLAttributes<HTMLDivElement>.
// ------------------------------------------------*/

// // type RechartsTooltipPayload = TooltipPayload[];
// type RechartsTooltipLabel = string | number;
...
//   label?: RechartsTooltipLabel;
//   formatter?: (value: any, name: any, entry: TooltipPayload, index: number) => React.ReactNode;
...
//       {!nestLabel ? tooltipLabel : null}
//       <div className="grid gap-1.5">
//         {payload.map((rawItem, index: number) => {
//           // item types from Recharts can vary; treat as any locally
...
//             >
//               {formatter && item?.value !== undefined && item.name ? (
//                 // formatter is called with (value, name, props, index)
...
// type RechartsLegendPayload = LegendPayload[];
...
//       {payload.map((entry: LegendPayload, idx: number) => {
//         const key = `${nameKey || ((entry as any).dataKey || entry.value) ?? idx}`;




//         const itemConfig = getPayloadConfigFromPayload(config, entry as any, key);
...
//               <itemConfig.icon />
//             ) : (
//               <div className="h-2 w-2 shrink-0 rounded-[2px]" style={{ backgroundColor: (entry as any).color }} />
//             )}
//             {itemConfig?.label}
//           </div>
//         );
...
function ChartLegendContent({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }: ChartLegendContentProps) {
  ...
  return (
    <div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((entry: LegendPayload, idx: number) => {
   const entryKey = (entry as any).dataKey ?? (entry as any).value;

// Step 2: nameKey ke saath fallback karo
const resolvedKey = nameKey || entryKey || idx;

// Step 3: template literal me use karo
